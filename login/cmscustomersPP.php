...
<h3 style="text-align:center;text-transform:uppercase">Paypal Customers</h3> 																			
<br>
<?php
...
$sql = "SELECT customer_id, customer_full_name, customer_phone,customer_email,date from  customersPP order by customer_id desc"; 
$messages = array();
	...
	if (willDelete) {
		...
		$.each(values, function (key, value) {
			...
			if(check.checked == true){
				...
				$.ajax({
				type:"POST",
				url: "delCustomerPPajax.php?id="+value,
...
function refresh(){
	...
	for(i=0;i<gr;i++){
		...
		read.className="nnE"
		read.href="cms.php?cms=customersPP3&&ID="+novosti[i].customer_id;
		...
		var read = document.createElement("a");
		read.className="nnP"
		read.href="cms.php?cms=customersPP2&&ID="+novosti[i].customer_id;
...
function search(a){
	if (a.length < 3) { 
		...
		$.ajax({
		type:"POST",
		url: "search.php?type=customerPP&&search="+b,
		contentType: "application/json; charset=utf-8",
		dataType: "JSON",
		success:function (data) {
