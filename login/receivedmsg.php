...
<h3 style="text-align:center;text-transform:uppercase">You received message</h3> 																			
<br>
<?php
$id=$_GET['id'];
$user = $_SESSION['id'];
mysqli_query($con,"UPDATE `messagestatus` SET type=1 WHERE message=$id and user = $user");
$sql = "SELECT ms.status,ms.type,m.id AS msgid,m.message as poruka, m.date, m.subject, a.user as sender, m.sender as sid
...
<span style="font-weight:bold"><?php echo $row["sender"]; ?></span>  
...
<a onclick="sendMsg(<?php echo $senderid; ?>)" class="nnE" style="padding:8px 20px;">REPLYS</a>
</div>
