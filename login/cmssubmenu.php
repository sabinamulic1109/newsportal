...
<h3 style="text-align:center;text-transform:uppercase">Add new submenu item</h3>
<form action="addsubmenu.php?id=<?php echo $grp; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">	 
<input type="text" name="naslov"   style="width:100%; height:40px; border:#941046 solid 1px; padding:5px; margin:5px 0;" placeholder="Menu"> 
<input type="text" name="url"   style="width:calc(50% - 15px); height:40px; border:#941046 solid 1px; padding:5px; margin:5px 0; float:left" placeholder="Enter URL">
...
<br>
<br>
<h3 style="text-align:center;text-transform:uppercase">Added submenus</h3>
<div class="col-sm-12" style="padding:0">
...
<table width="100%" border="0" id="tablesubmenu">
<tr>
<td width="80%" style="border-bottom:#000000 solid 2px;"><div align="left">Title</div></td>
 <td width="20%" style="border-bottom:#000000 solid 2px;"><div align="left">Action</div></td>
</tr>				
<?php

$sql = "SELECT * from podgrupe where grupa='$grp' order by id"; 
$result = mysqli_query ($con,$sql); 
$submenu = array();
while($row = mysqli_fetch_array($result)){
	$submenu[] = $row;
...
$naslovS=$row["naziv"];
echo '<tr>
<td style="border-bottom:#999999 solid 1px;"><div align="left"><p><strong>',$naslovS,'</strong></p></div></td>
<td style="border-bottom:#999999 solid 1px;"><div align="left">
<div class="nn"><a href="cms.php?cms=submenu2&&id=',$idDok,'"><span style="color:#941046;">Edit</span></a></div>
<div class="nn"><a href="delsubmenu.php?id=',$idDok,'" onclick="return checkDelete('.$idDok.')"><span style="color:#941046;">Delete</span></a></div>
</div></td>
</tr>';	 */			
}
?>						
...
function checkDelete(id){
	event.preventDefault();
		...
		if (willDelete) {
			window.location.href = "delsubmenu.php?id="+id;
      } else {
...
function refresh(){
	...
	var tabela = document.getElementById('tablesubmenu');
	$('#tablesubmenu').empty();
	...
	var menu = <?php echo json_encode($submenu,true); ?>;
	...
	for(i=0;i<gr;i++){
		...
		edit.href = "cms.php?cms=submenu2&&id="+menu[i].id;
...
function search(a){
	if (a.length < 3) { 
		refresh();
		$('#pretraga').css('display','none');
		$('#tablesubmenu').css('display','table');
		$('#pagesN').css('display','block');
	} else {
		b = a.replace(" ",".");
		$.ajax({
		type:"POST",
		url: "search.php?type=submenu&&search="+b+"&&grupa=<?php echo $grp; ?>",
