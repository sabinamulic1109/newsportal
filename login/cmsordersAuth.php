...
<h3 style="text-align:center;text-transform:uppercase">Authorize.net Orders</h3> 																			
<br>
<?php
	...
	if (willDelete) {
		...
		$.each(values, function (key, value) {
			...
			if(check.checked == true){
				...
				$.ajax({
				type:"POST",
				url: "delOrderAuthajax.php?id="+value,
...
function refresh(){
	...
	for(i=0;i<gr;i++){
		...
		read.className="nnE"
		read.href="cms.php?cms=ordersAuth2&&ID="+novosti[i].order_id;
...
function search(a){
	if (a.length < 3) { 
		...
		$.ajax({
		type:"POST",
		url: "search.php?type=ordersAuth&&search="+b,
		contentType: "application/json; charset=utf-8",
		dataType: "JSON",
		success:function (data) {
